...
gcontainer_node_pool { "autoscaling-pool":
  ensure             => present,
  initial_node_count => 2,
  cluster            => "mycluster-${cluster_id}",
  config             => {
    machine_type => 'n1-standard-2', # we want a 4-core machine for our cluster
    disk_size_gb => 500,             # ... and a lot of disk space
  },
  autoscaling        => {
    enabled => true,
